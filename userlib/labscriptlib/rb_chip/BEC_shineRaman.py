...
## prepping all hardware settings
   ...
   # ni_pci_6733_1 analog outputs:
    ...
    AOM_Raman_1.setfreq(t, 86.903*MHz)
    AOM_Raman_1.setamp(t, .7077)
    AOM_Raman_2.setfreq(73000000)
...
def ARP_RF(t):
    # ni_usb_6343_1 analog outputs:
    offset.customramp(t, 20*ms, HalfGaussRamp, Offset+dTpzShim+0, Offset+dTpzShim+0.5,  -10*ms, samplerate = 1/step_ramp_RF)
...
def dipole_hold_insitu(t):
    #ni_pci_6733_0 analog outputs:
    if tube_trap:
        dipole_intensity.customramp(t, TimeDipoleUnload, LineRamp, FinalDipoleInSitu, 0.19, samplerate=1/step_hold)
    else:
        dipole_intensity.customramp(t, TimeDipoleUnload, LineRamp, FinalDipoleInSitu, FinalDipoleRelease, samplerate=1/step_hold)
    
    dipole_split.customramp(t, TimeDipoleUnload, LineRamp, FinalDipoleSplit, 0.0, samplerate=1/step_hold)
    
    # ni_usb_6343_1 analog outputs: ramp off field for imaging
    if uwave_transfer and rf_transfer:
        x_bias.customramp(t, 15*ms, LineRamp, TpxShimEnd, TpxShim, samplerate=1/step_hold, units='A')
        y_bias.customramp(t, 15*ms, LineRamp, TpyShimEnd, TpyShim, samplerate=1/step_hold, units='A')
        z_bias.customramp(t, 15*ms, LineRamp, a0*TpzShimEnd, a0*TpzShimImg, samplerate=1/step_hold, units='A')
        offset.customramp(t, 15*ms, LineRamp, Offset+dTpzShim+dTpzRaman, 0, samplerate=1/step_hold)
...
@stage
def pulse_raman(t):
    shutter_raman1.go_high(t-10e-5)
    shutter_raman2.go_high(t-10e-5)
    raman_1.go_high(t)
    mixer_raman_1.constant(t,0.2)
    raman_2.go_high(t)
    mixer_raman_2.constant(t,0.2)
...
def insitu_open_shutter(t):
    ...
    shutter_x_insitu.go_high(t)        #shutter_x if along XY
    shutter_top_repump.go_high(t - 0.0*ms)
...
def free_TOF(t):
    ...
    if uwave_transfer and rf_transfer:
        z_bias.customramp(t, 1e-3, LineRamp, a0*TpzShimEnd, a0*TpzShimImg, samplerate=1/step_TOF, units='A')
        offset.customramp(t, 15*ms, LineRamp, Offset+dTpzShim+dTpzRaman, 0, samplerate=1/step_TOF)
    return 7e-3
