...
## prepping all hardware settings
   ...
   # ni_pci_6733_1 analog outputs:
    ...
    # use 13.9MHz ARP split
    AOM_Raman_1.setfreq(t, 87*MHz)
    AOM_Raman_1.setamp(t, 1.0)
    AOM_Raman_2.setfreq(73100000-4*3678)
...
def dipole_evaporation_2(t):
    ...
    # Keep high field to RF transfer:
        ...
        offset.customramp(t, 300*ms, LineRamp, Offset + dTpzShim, Offset+dTpzShim - 0.5, samplerate = 1/step_ramp_Bfield)
    else:
        x_bias.constant(t, TpxShim, units='A')
...
def ARP_RF(t):
    # ni_usb_6343_1 analog outputs:
    offset.customramp(t, 100*ms, HalfGaussRamp, Offset+dTpzShim-0.5, Offset+dTpzShim, 50*ms, samplerate = 1/step_ramp_RF)
...
def free_TOF(t):
    ...
    dipole_intensity.constant(t-short_TOF, 0)
    dipole_split.constant(t, 0)
    probe_2.constant(t, ProbeInt)
