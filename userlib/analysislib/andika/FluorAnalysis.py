...
filename = foldername + fileheader + str("%.2d" % idEnd) + ".h5"
...
def getODcorr_shifted(camID):
    Isat = 297;
    alpha = 1;
    img_abs = np.ma.masked_invalid(array(h5_file['data'][camID]['Raw'])[1])
    img_pro = np.ma.masked_invalid(array(h5_file['data'][camID]['Raw'])[2])
    img_bkg = np.ma.masked_invalid(array(h5_file['data'][camID]['Raw'])[3])
    return np.flipud(-alpha*np.log((img_abs-img_bkg)/(img_pro-img_bkg)) - ((img_abs-img_pro)/Isat))
...
def getODbg(camID):
    return np.flipud(array(h5_file['data'][camID]['Raw'])[1])
...
with h5py.File(filename) as h5_file:
    ...
    Image = {}; Background = {}; ImgFluor = {};
    Image["OptDepth"] = getODcorr(camID)
    ImgFluor["OptDepth"] = np.flipud(array(h5_file['data'][camID]['Raw'])[0])
    Background["OptDepth"] = np.flipud(array(h5_file['data'][camID]['Raw'])[2])
...
Image["xVals"] = xVals;
Image["yVals"] = yVals;
ImgFluor["xVals"] = xVals;
ImgFluor["yVals"] = yVals;
...
z0c = np.array([294,270])
#z0c = np.array([285,251])
cropsize = np.array([40,40])
...
n_bg,bins,patches = ax1.hist(Background["OptDepth"].ravel(), bins=np.linspace(0.5,100.5,101))
ax3.plot(np.linspace(1,100,100),n_bg,'ro-')
ybg = n_bg
xbg = np.linspace(1,100,100)


pguess = lmfit.Parameters()
pguess.add('A', value = n_bg.sum())
pguess.add('lamb', value = 1.0)
pguess.add('xscale', value = 10.0)
fit_par = Qfunc.FitQgas(xbg, ybg, Qfunc.poisson_dist, pguess)
fitdata = Qfunc.poisson_dist(xbg, **fit_par.values)
ax3.plot(xbg,fitdata,'rx-')

print (fit_par.fit_report())

bgmean = (fit_par.values['lamb']*fit_par.values['xscale'])



CroppedImg = Util.ImageCrop(Image['xVals'], Image['yVals'], Image['OptDepth'], z0c, cropsize, Scaled = True, Center = False)
#CroppedBkg = Util.ImageCrop(Image['xVals'], Image['yVals'], Background['OptDepth'], z0c, cropsize, Scaled = True, Center = False)
n_im,bins,patches = ax2.hist(CroppedImg["OptDepth"].ravel(), bins=np.linspace(0.5,100.5,101))
ax2.plot(np.linspace(1,100,100),n_im,'ro-')

pguess.add('A', value = n_im.sum())
pguess.add('lamb', value = 1.0)
pguess.add('xscale', value = 10.0)
fit_par = Qfunc.FitQgas(xbg, n_im, Qfunc.poisson_dist, pguess)
fitdata = Qfunc.poisson_dist(xbg, **fit_par.values)
ax2.plot(xbg,fitdata,'rx-')
print (fit_par.fit_report())
immean = (fit_par.values['lamb']*fit_par.values['xscale'])

#CroppedImg["OptDepth"][CroppedImg["OptDepth"]<(bgmean*3)] = 0;
imshowImg(CroppedImg, CroppedImg["OptDepth"].min(), CroppedImg["OptDepth"].max(), ax4, False)

CroppedImgFluor = Util.ImageCrop(ImgFluor['xVals'], ImgFluor['yVals'], ImgFluor['OptDepth'], z0c, cropsize, Scaled = True, Center = False)
imshowImg(CroppedImgFluor, CroppedImgFluor["OptDepth"].min(), CroppedImgFluor["OptDepth"].max(), ax5, True)

#plt.tight_layout()

zero = phys.NumberCalcSum(CroppedImgFluor['OptDepth'],1)
zero = CroppedImgFluor['OptDepth'].max()
print zero

#run.save_result('zero', zero)
#run.save_result('peak_OD', peakOD)
'''
print_time('Do cropping...')
# CROPPED IMAGE SHOW
p1c = np.array([160,240])
z0c = np.array([285,265])
m1c = np.array([485,250])
cropsize = np.array([80,80])
