...
A batch processor designed to study the intensity dependence of absorption images
...
def BatchProcess(FileTuple, PulseTime):
    ...
    r0 = numpy.array([0,0]); # Note that this is python ordered so [0] is y and [1] is x
...
#        fig.canvas.draw();
    ...
    for ISat in ISatCounts:
        ODCorrect = CorrectOD(Results["OD"], Results["Counts"], PulseTime, ISat, tau);
