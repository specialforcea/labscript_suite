...
import pandas as pd
import numpy as np
from analysislib.common.dika_fitting import *
...
#params, u_params = fit_sine(pulsetime, Nfrac3);
#print params
#xfit = linspace(min(pulsetime),max(pulsetime),len(pulsetime)*3)
#yfit = sine(xfit, params[0],params[1],params[2],params[3])

#params, u_params = fit_res_sine(pulsetime, Nfrac3);
#print params
#xfit = linspace(min(pulsetime),max(pulsetime),len(pulsetime)*3)
#yfit = res_sine(xfit, params[0],params[1])

params, u_params = fit_sine_fixed_decay(pulsetime, Nfrac3);
print params
xfit = linspace(min(pulsetime),max(pulsetime),len(pulsetime)*3)
yfit = sine_fixed_decay(xfit, params[0],params[1],params[2])

#plot(offBias,Nfrac2,'bo')
#plot(runNo,Nfrac2,'bo')
