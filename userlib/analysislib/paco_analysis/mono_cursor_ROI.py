...
class cursors_ROI(object):
    def __init__(self, z, x = None, y = None, n_ROI=1):
        ...
        self.shot.pcolormesh(self.z, cmap = 'viridis', vmin=-0.0, vmax=0.4)
    ...
    def click(self, event):
        ...
        if clickCounter < 4*self.n_ROI:
            ...
            np.save(r'C:\labscript_suite\userlib\analysislib\paco_analysis\ROI_temp.npy', gathered_data)
    ...
    with h5py.File(fpath) as h5_file:
        ...
        for appended, id in image_group.iteritems():
            ...
            if len(h5_file['data'][appended]['Raw'][:]) != 0:
                atoms[id] = (np.array(h5_file['data'][appended]['Raw'])[0])
                probe[id] = (np.array(h5_file['data'][appended]['Raw'])[1])
                bckg[id] =  (np.array(h5_file['data'][appended]['Raw'])[2])
